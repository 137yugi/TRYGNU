...
| SP横 | 画面ドラッグで移動、右下 `SNAP`、右上 `メニュー` |
| SP縦 | 上部に16:9ゲーム画面、下部に `開始/再開` / `スナップ` / `メニュー` |

詳しい操作は [docs/controls.md](docs/controls.md) を参照してください。

## ラン中の要素

- ビルド: メニューからジョブ4種と武器4種を選べます。
...
- 変異: 10ウェーブごとの2択強化です。
- 装備比較: Diablo風のコア装備です。白コモン、青マジック、黄色レア、紫エピック、オレンジレジェンダリー、赤エンシェントの6段階で、37種類のアフィックスから複数付与されます。装備効果はレベルアップ能力とは別枠で重複します。
