...
| 環境 | 操作 |
| --- | --- |
| キーボード | `WASD` / 矢印で移動、`Enter` で開始/選択、`M` でメニュー、`Esc` で閉じる、`1/2/3` で選択 |
| 追加キー | `F` でフルスクリーン、`H` でデバッグHUD |
| マウス | キャンバスをクリック/ドラッグして移動先指定 |
| SP横 | ステージ全画面。指を置いた位置からの相対ドラッグで移動し、右上 `メニュー` / `全画面` を使う |
| SP縦 | ステージ全画面。相対ドラッグ移動、下部オーバーレイに `開始/再開` / `メニュー` |

詳しい操作は [docs/controls.md](docs/controls.md) を参照してください。

## スマホ / Safari / PWA

iPhone/iPad Safari の通常タブでは、アドレスバーやタブバーをWebページ側から完全には消せません。スマホ運用では Safari の共有メニューから「ホーム画面に追加」して起動すると、`manifest.webmanifest` と Apple standalone meta によりブラウザUIを抑えた表示で遊べます。通常タブで遊ぶ場合も、縦横回転と safe-area に合わせてステージと操作ボタンが欠けにくいようにしています。

縦持ちは縦長ステージ、横持ちは横長ステージとして扱います。回転後に表示が詰まった場合は、いったん指を離して数秒待つか、Safari のバーが畳まれるまで軽くスクロールせず画面をタップしてから再開してください。

## ラン中の要素

- ビルド: メニューから闘士タイプ8種と呪鎖武器タイプ8種を選べます。
- 呪鎖ヌンチャク: 挑戦者の移動でヘッドを引っ張り、慣性とテンションで振り回して敵へ当てる主武器です。
- 契約目標: 撃破、ノーダメージなどの興行チャレンジです。
