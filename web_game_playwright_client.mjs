...
async function doChoreography(page, canvas, steps) {
  for (const step of steps) {
    if (step.click_selector) {
      try {
        await page.click(step.click_selector, { timeout: step.timeout_ms || 3000 });
      } catch (err) {
    ...
    if (typeof step.click_page_x === "number" && typeof step.click_page_y === "number") {
      await page.mouse.click(step.click_page_x, step.click_page_y);
    }
