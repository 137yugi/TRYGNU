...
- `combat.skill_stacks`: スキルIDごとの取得回数。分裂ヌンチャク、高速回転、反射、衝撃波、連鎖、丸鋸、重力、低HP過給、会心、処刑、吸命、状態異常などの重複確認に使います。
- `combat.phantom_nunchaku`: 分裂ヌンチャク数。
...
- `phantoms[]`: 各分裂ヌンチャクの `x/y/prev_x/prev_y/speed/r`。描画/QAで本体外の追加ヘッドを追うための配列です。
...
- `window.set_nunchaku_stretch_limit(value)`: QA用にヌンチャク最大長を 88-220 の範囲へ丸めて変更します。戻り値はありません。
