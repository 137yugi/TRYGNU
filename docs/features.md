...
| スタック可能スキル | レベルアップ3択、変異 | 実装済み | skill loop、状態JSON | `combat.skill_stacks` に分裂導電線/高速回転/反射/衝撃波/連鎖/丸鋸/重力/低HP過給/会心/処刑/吸命/状態異常などの重複数が出る |
...
| Diablo風装備 | ウェーブ報酬回収、pickup compare | 実装済み | `npm run test:equip` | 神経殻装備/導電線装備として6レア度、37アフィックス、複数affix、`inventory.equipment_slots/equipment_mods` が出る |
| 神経殻/導電線装備分離 | pickup compare, state JSON | 実装済み | `npm run test:equip` | 内部スロット `body` と `nunchaku` の2枠が別々に保持され、合算modが再計算される |
...
| ノイズ群ロール | chaser / stalker / bruiser / zoner | 実装済み | longrun、状態JSON | `enemies[].role` が観測でき、ノイズ群数がcap内に収まる |
| 過負荷中枢/無限ウェーブ | Wave15、以後10waveごと、`?boss_debug=1`、`?phase3_debug=1` | 実装済み | `npm run test:longrun` | `run.boss` が出現し、撃破で `run.boss_kills` が増え、`run.next_boss_wave` が更新され、`mode: running` のまま継続する |
...
| ドット調ビジュアル | `public/assets/pixel`, Phaser preload | 実装済み | responsive screenshots / `npm run test:equip` | ニューロン核タイプ/電脈導線タイプ/ノイズ群/装備/ドロップが画像アセットで描画される |
...
| セーブデータ移行 | 未実装 | SYNAPSE STORM再構築により旧 `game.js` 版からの移行処理なし | 旧キーが残る環境で副作用がないか確認 |
