...
メニューの `ライブ入力` を開くと通常プレイヤーは `TikTok ID` だけを入力できます。`@yrachac` のように `@` 付きで入れても保存時に整形されます。詳細WSS、内部の連携合言葉、端末接続ページ、Safari直結診断、受信テストは通常メニューでは見せず、運営/QA確認時だけ `?admin=1` で表示します。
...
- `browserRelay`: Smart Connect設定または管理者の詳細WSSに入力した外部WSSリレー
...
ただしゲーム本体にはサーバーを追加せず、スマホから公開URLを開くだけで使える受け口として Smart Connect を実装しています。通常UIではTikTok IDだけを入力し、ゲームは `public/config/live-relay.json` の `browserRelayUrl` / `relayUrl` / `wssUrl` / `url` を読み、ブラウザ標準の `WebSocket` でそのリレーへ接続します。`{room}` / `{user}` / `{username}` / `{tiktok}` は保存済みTikTok ID、`{channel}` は内部チャンネルに置換されます。接続直後には互換リレー向けに `{ type: "subscribe", room, username, channel }` を送ります。WSS未設定時は通常プレイヤーへ技術語を出さず、`ライブ接続準備中` と表示します。

`public/config/live-relay.json` の最小形:

```json
{
  "enabled": true,
  "browserRelayUrl": "wss://relay.example/live/{room}",
  "statusLabel": "ライブ接続準備中"
}
```

QA/運営だけは `?admin=1` で詳細WSS欄を表示し、設定ファイルを使わず一時的なWSSを直接入力できます。URLクエリ `?live_relay_config=./config/live-relay.json` で設定ファイルを差し替え、`?live_relay_config=off` でSmart Connect設定読み込みを止められます。
...
`public/tiktok-direct-spike.html` は、iOS/iPad Safari/PWAだけでTikTok Live信号を直接受けられるかを証拠付きで確認する検証ページです。`?room=yrachac&autorun=1` で自動診断し、結果は画面のJSONと `localStorage.tiktok_direct_probe_last_v1` に残ります。成功条件は `DIRECT_OK` です。失敗理由は `CORS_BLOCKED`、`SIGNED_WS_REQUIRED`、`BROWSER_WS_BLOCKED`、`TIKTOK_OFFLINE`、`UNKNOWN` のいずれかに分類します。自動テストでは `?mock=cors` を使い、3viewportでJSONとUI表示を検証します。標準コマンドは `npm run test:live:direct-spike` です。
